...
export const LoginScreen = ({ navigation }) => {
  ...
  const Pname = personelName.map((p) => p.Password); 
  const Ppass = personelName.map((p) => p.Username); 
  ...
  const personel = [Pname, Ppass];//Personel bilgileri sonrasında kullancağımız kodlar için dizi içine alındı.
